...
NexusCart is a fault-tolerant, distributed e-commerce backend built using **Spring Boot** and **Spring Cloud**. It handles catalog management, cart operations, stock reservation, and wallet payments using an event-driven Saga pattern for distributed transaction management and Resilience4j circuit breakers for high availability.
...
```
[ Client / Talend ] ---> [ API Gateway :8080 ]
                               |
       +-----------------------+-----------------------+
       |                       |                       |
[ Shop Service ]      [ Inventory Service ]    [ Wallet Service ]
   (:8081)                 (:8082)                  (:8083)
       |                       |                       |
       +-----------------------+-----------------------+
                               |
...
| **API Gateway** | `8080` | Unified edge proxy, request routing, and client entry point |
| **Shop Service** | `8081` | Manages Products, Carts, Orders, and Saga orchestration |
| **Inventory Service** | `8082` | Manages stock availability, stock reservation, and restorations |
| **Wallet Service** | `8083` | Handles user digital wallets, deposits, and payment deductions |
...
* **Unified API Gateway Routing:** All client traffic is routed dynamically through Spring Cloud Gateway to backend services registered with Eureka.
...
* **Database:** MySQL / H2
* **API Testing & Automation:** Talend API Tester / Postman
